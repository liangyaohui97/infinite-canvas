...
+ [优化] 首次启动、页面资源加载和画布本地数据恢复统一显示全局 Loading，模型生成等业务请求不触发该提示，并延迟加载画布导入导出依赖。
+ [优化] 前端按页面和重型弹层延迟加载，并为生产静态资源生成和协商 Brotli、gzip 压缩版本，降低首次访问下载体积。
+ [修复] 模型选择菜单会按完整名称自适应宽度并单独展示渠道，在深浅主题和窄视口下均保持选项可读。
+ [修复] 同一文本节点反复生成时，新结果会在父节点右侧寻找空位，避免兄弟分支相互遮挡。
+ [修复] 模型列表及文本、生图、视频、音频和自定义脚本请求统一通过同源 AI API 代理转发，避免新增渠道被浏览器跨域策略拦截。
+ [修复] 渠道地址已包含 `/v1`、`/v3` 或 `/v1beta` 等版本段时不再重复追加版本路径，避免请求落到不存在的接口。
